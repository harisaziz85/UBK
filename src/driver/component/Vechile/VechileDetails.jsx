...
const VehicleDetailsPage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/common/document/create`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      // Refetch vehicle to update documents
      await fetchVehicle();
    } catch (err) {
      console.error(err);
  ...
  if (error || !vehicle) {
    return (
      <>
        <p className="p-6">Vehicle not found or error loading.</p>
        <ToastContainer position="top-right" />
      </>
    );
  }
  ...
  const canUploadDocuments = ['upload', 'both'].includes(accessType);
  const showDocumentsSection = accessType !== 'none';
                  ...
                  <div className="bg-white rounded-lg p-6 w-full max-w-md">
                    ...
                    <div className="flex justify-end gap-2 mt-6">
                      ...
                      <button 
                        ...
                        className="px-4 py-2 bg-[#043677] text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-700 transition-colors"
                      >
